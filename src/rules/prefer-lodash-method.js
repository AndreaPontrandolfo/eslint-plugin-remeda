...
module.exports = {
  ...
  create(context) {
    ...
    function isStaticNativeMethodCall(node) {
      ...
      return (
        (callerName in staticMethods &&
          includes(staticMethods[callerName], methodName)) ||
        isNonNullObjectCreate(callerName, methodName, node.arguments[0])
      );
    }

    function isNativeStringMethodCall(node) {
      const lodashFunction = nativeStringMap[getMethodName(node)];
      return Boolean(lodashFunction) && methodExists(lodashFunction);
    }

    function canUseLodash(node) {
      return (
        isNativeCollectionMethodCall(node) ||
        isStaticNativeMethodCall(node) ||
        isNativeStringMethodCall(node)
      );
    }
    ...
    function shouldIgnore(node) {
      return (
        someMatch(ignoredMethods, getMethodName(node)) ||
        someMatch(ignoredObjects, getTextOfNode(getCaller(node)))
      );
    }
    return assign(
      {
        ...
        "CallExpression:exit"(node) {
          if (
            ...
            context.report({
              node,
              message: `Prefer '_.${lodashMethodName}' over the native function.`,
            });
          }
        },
      },
