...
module.exports = {
  ...
  create(context) {
    ...
    function reportIfConnectiveOfParamInvocations(node) {
      ...
      if (
        retVal &&
        retVal.type === "LogicalExpression" &&
        (retVal.operator === "&&" || retVal.operator === "||")
      ) {
        if (
          isOnlyParamInvocationsWithOperator(retVal, paramName, retVal.operator)
        ) {
          context.report({
            node,
            message,
            data: reportConstants[retVal.operator],
          });
        }
      }
    }
    ...
    function reportIfDoubleFilterLiteral(callType, node, version) {
      if (
        ...
        isCallToConditionMethod(getMethodName(node.parent.parent), version) &&
        usesShorthandInChain(node.parent.parent)
      ) {
